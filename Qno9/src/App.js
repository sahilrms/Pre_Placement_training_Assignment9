...
function App() {
  ...
  let remainingTodos=todos.filter(item=>item!==todo)
    setTodos(remainingTodos);
    let filteredStatus=todoStatus.filter((item,idx)=>idx!==index);
    setTodoStatus(filteredStatus)
  }
  ...
  function updateTodo(index){
      ...
      updatedStatus[index] =
        updatedStatus[index] === 'pending' ? 'done' : 'pending';
      ...
      <div className="flex flex-wrap justify-center items-center w-screen gap-4 py-6">
        {todos?.map((item,index)=>{
          return(<div key={item} className="flex flex-col py-4 px-4 w-3/12 h-fit bg-blue-950 shadow-lg capitalize rounded-2xl">
           ...
           <button className="my-4 w-full h-12 bg-blue-800 text-white rounded-lg " onClick={()=>updateTodo(index)}>Update Status</button>
           <button className="my-4 w-full h-12 bg-blue-800 text-white rounded-lg" onClick={()=>removeTodo(item,index)}>Remove</button>
